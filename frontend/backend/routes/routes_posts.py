...
def pin_post(post_id: int):
    """置頂貼文（僅campus_admin和dev_admin可操作）"""
    try:
        from utils.auth import get_effective_user_id
        uid = get_effective_user_id()
        if not uid:
        ...
        with get_session() as s:
            ...
            if post.is_pinned:
                ...
                return _wrap_ok({
                    ...
                    "pinned_by": (user.username if user else None)
                })
            ...
            s.commit()
            log_user_action(
                event_type="content.post.pin",
                actor_id=uid,
                actor_name=(user.username if user else ""),
                action=f"置頂貼文 #{post_id}",
                target_id=post_id,
                target_type="post",
                session=s,
            )
            ...
            return _wrap_ok({
                "message": "貼文已置頂",
                "post_id": post_id,
                "pinned_at": post.pinned_at.isoformat(),
