...
>>>>>>> Stashed changes
    ...
    const filteredTickets = [...(supportData?.tickets || [])];
      if (filter.status !== 'all') {
        sorted = sorted.filter(t => t.status === filter.status);
      }
      if (filter.category !== 'all') {
        sorted = sorted.filter(t => t.category === filter.category);
      }
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <select 
                  ...
                  className="w-full text-sm rounded-lg border-border bg-surface-hover focus:ring-primary focus:border-primary"
                >
              ...
              <div>
                <label className="block text-xs text-muted mb-2">分類</label>
                <select 
              ...
              <MobileAdminCard
                ...
                subtitle={
                  <div className="flex items-center gap-2 text-xs text-muted">
                    ...
                    <div className="text-xs font-medium text-muted">{new Date(ticket.last_activity_at).toLocaleDateString()}</div>
                    <div className={`text-xs font-medium ${getStatusColor(ticket.status)}`}>{ticket.status.toUpperCase()}</div>
                    <div className="text-xs font-medium text-muted">{ticket.category}</div>
                  </div>
                }
