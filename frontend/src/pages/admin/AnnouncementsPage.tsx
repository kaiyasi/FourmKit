...
export default function AnnouncementsPage(){
  ...
  const submit = async () => {
    ...
    try {
      ...
      const j = await r.json().catch(()=>({}))
      if (!r.ok || j?.ok === false) throw new Error(j?.msg || '發布失敗')
      ...
      <main className="mx-auto max-w-5xl px-3 sm:px-4 pt-20 sm:pt-24 md:pt-28 pb-24 md:pb-8">
        ...
        <div className="bg-surface border border-border rounded-2xl p-4 sm:p-6 shadow-soft">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={load} className="btn-ghost px-3 py-2 flex items-center gap-2 text-sm"><RefreshCw className="w-4 h-4"/> 重新載入</button>
          </div>
          {loading ? (
            ...
            <div className="space-y-3">
              {items.map((a, i) => (
                <div key={i} className="p-3 rounded-xl border border-border bg-surface-hover">
                  <div className="font-medium text-fg">{a.title}</div>
                  <div className="text-sm text-muted whitespace-pre-wrap mt-1">{a.description}</div>
                </div>
              ))}
            </div>
          )}
