#!/usr/bin/env bash
# ===============================================================================
# 🚀 Serelix Studio ForumKit - 開發建制腳本
# ===============================================================================
# 功能：清理 -> 建置 -> 部署 -> 初始化 -> 驗證
...
header() {
    printf "\n${MAGENTA}═══════════════════════════════════════════════════════════════════════════════${RESET}\n"
    printf "${BOLD}${CYAN}$ICON_ROCKET ForumKit Development Rebuild${RESET}\n"
...
# STEP 7.5: 初始化聊天房間
step "$ICON_INFO 初始化聊天房間"
if [ "$backend_ready" = "true" ]; then
    docker compose exec -T backend python scripts/init_chat_rooms.py >/dev/null 2>&1 && \
        success "聊天房間初始化完成" || warning "聊天房間初始化失敗"
else
    docker compose run --rm backend python scripts/init_chat_rooms.py >/dev/null 2>&1 && \
        success "聊天房間初始化完成" || warning "聊天房間初始化失敗"
fi
