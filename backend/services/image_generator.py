...
class ImageGenerator:
    ...
    def _add_timestamp(
        ...
        # 格式化時間文字
        time_format = timestamp_config.get('format', 'relative')
        ...
        if time_format == 'relative':
            ...
            if seconds < 60:
                ...
                time_text = f'{seconds // 60}分鐘前'
            elif seconds < 86400:
                time_text = f'{seconds // 3600}小時前'
            else:
            ...
            strftime_format = format_mapping.get(time_format, time_format)
            try:
                time_text = created_at.strftime(strftime_format)
                # 如果使用 %p (am/pm)，則轉換為小寫
