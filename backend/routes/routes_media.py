"""
Module: backend/routes/routes_media.py
Unified comment style: module docstring + minimal inline notes.
"""
from flask import Blueprint, request, jsonify, abort, send_file, redirect
...
def serve_media_file(media_id):
    """提供媒體檔案下載
    - approved 媒體：公開存取
    - pending/rejected 媒體：需要審核權限或作者本人
    """
    ...
    with get_session() as s:
        ...
        # 若為已核准媒體，直接轉址至公開 /uploads 路徑，避開 API 限流與憑證檢查
        if (media.status or '').lower() == 'approved':
            try:
                rel = resolve_or_publish_public_media(media.path or '', int(media.id), getattr(media,'mime_type',None))
                if rel and rel.startswith('public/'):
                    return redirect(f"/uploads/{rel}", code=302)
            except Exception:
                pass
        ...
        try:
            ...
            if not (user and (can_moderate_content(user, media.school_id) or media.author_id == uid)):
                abort(403)
        except Exception:
            abort(401)
